...
class MultiViewCNNForwardPass(ForwardPass):
    ...
    def __init__(
        ...
        # )
        D = self._generation_params.depth_planes
        self.s_gpu = np.zeros((self.rays_batch, D), dtype=np.float32)
        
        self.points_gpu =  np.zeros((self.rays_batch, D, 4), dtype=np.float32)
    ...
    def forward_pass(self, scene, images_range):
        ...
        while self.ref_idx < end_img_idx:
            ...
            features_gpu = features.ravel()
            ray_idxs_gpu = ray_idxs.astype(np.int32)
            P_gpu = np.array(P).ravel()
            P_inv_gpu = P_inv.ravel()
            camera_center_gpu = camera_center
...
class MultiViewCNNVoxelSpaceForwardPass(ForwardPass):
    ...
    def __init__(
        ...
        M = self._generation_params.max_number_of_marched_voxels
        self.s_gpu = np.zeros((rays_batch, M), dtype=np.float32)
        self.ray_voxel_count_gpu =  np.zeros((rays_batch,), dtype=np.int32)
        self.ray_voxel_indices_gpu = np.zeros((rays_batch, M, 3), dtype=np.int32)
        self.voxel_grid_gpu = None
    ...
    def voxel_grid_to_gpu(self, scene):
        ...
        self.voxel_grid_gpu = scene.voxel_grid(
                self._generation_params.grid_shape
            ).transpose(1, 2, 3, 0).ravel()
...
class RayNetForwardPass(ForwardPass):
    def __init__(
        ...
        # Initialize the ray to occupancy accumulators as GPU arrays
        self.ray_to_occupancy_accumulated_pon = np.ones(
                tuple(grid_shape),
                dtype=np.float32
            ) * (np.log(gamma) - np.log(1 - gamma))
        
        self.ray_to_occupancy_accumulated_out_pon = np.ones(
                tuple(grid_shape),
                dtype=np.float32
            ) * (np.log(gamma) - np.log(1 - gamma))
    ...
    def forward_pass(self, scene, images_range):
        ...
        for it in range(bp_iterations):
            ...
            for ref_idx in range(start_img_idx, end_img_idx, skip):
                # Get the valid rays for the current image
                ray_idxs = self.get_valid_rays_per_image(scene, self.ref_idx)
